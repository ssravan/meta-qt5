...
info(EGLDisplay egl_dpy)
{
   ...
   printf("GL_VENDOR: %s %x\n", (char *) glGetString(GL_VENDOR), glGetError());
   printf("GL_VERSION: %s %x\n", (char *) glGetString(GL_VERSION), glGetError());
   printf("GL_RENDERER: %s\n", (char *) glGetString(GL_RENDERER), glGetError());
              ...
              EGLSurface *surfRet)
{
   ...
   *surfRet = eglCreateWindowSurface(egl_dpy, config, (void*)win, NULL);
...
main(int argc, char *argv[])
{
   ...
   egl_dpy = eglGetDisplay((int)x_dpy);
   if (!egl_dpy) {
   ...
   make_x_window(x_dpy, egl_dpy,
                 "ES info2", 0, 0, winWidth, winHeight, es_ver,
                 &win, &egl_ctx, &egl_surf);
   ...
   fprintf(stderr, "createProgram: %x\n", prog);
